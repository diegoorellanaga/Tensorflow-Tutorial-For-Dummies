# -*- coding: utf-8 -*-
"""
Created on Mon Oct 23 12:02:49 2017
...
num_input = 2 # The input data has 1 dimension
num_classes = 2 # The output has 1 dimension
...
def create_data_label(dim,amount,input_data):
    ...
    for i in range(dim):
         ...
         matrix[:][i]=np.sin(input_data[:][i])+np.random.randn(1,amount)/16
    return matrix
...
with tf.Session() as sess:
    ...
    plt.plot(input_test.T[:][0],output_test.T[:][0],'b--')
    plt.plot(input_test.T[:][0],results.T[:][0],'ro')
    plt.show()
    plt.plot(input_test.T[:][0],output_test.T[:][1],'b--')
    plt.plot(input_test.T[:][0],results.T[:][1],'ko')
    plt.show()
...
#            sess.run(train_op, feed_dict={X: batch_x, Y: batch_y})
        ...
        if step % display_step == 0 or step == 1:
           ...
           # plt.plot(input_test.T[:][0],output_test.T[:][0],'b--')
            plt.plot(input_test.T[:][0],results.T[:][0],'ro')
            plt.show()
            #plt.plot(input_test.T[:][1],output_test.T[:][1],'b--')
            #plt.plot(input_test.T[:][1],results.T[:][1],'ko')
            #plt.show()
    ...
    matrix=np.ndarray([num_input,40000])*0
    matrix[:][0]=np.random.randint(-6000,6000,[1,40000])/300.0
    matrix[:][1]=matrix[:][0]
    matrix.sort(1)
    resultsfinal = sess.run(logits, feed_dict={X:matrix.T})
    matrix_final=create_data_label(num_input,40000,matrix).T
    plt.plot(matrix[:][0],matrix_final.T[:][0],'b-')
    plt.plot(matrix[:][0],resultsfinal.T[:][0],'ro')
    plt.plot(matrix[:][0],matrix_final.T[:][1],'b-')
    plt.plot(matrix[:][0],resultsfinal.T[:][1],'ko')
